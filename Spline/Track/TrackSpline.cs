...
{
    public static TrackSpline Track { get; private set; }
    ...
    public float meshDetail = 1;
    public float collisionDetail = 1;
    ...
    {
        Track = this;
        lanes = new LaneData[laneCount];

        float spacing = trackWidth / (laneCount * 2);
        float pos = -trackWidth / 2;
        for (int i = 0; i < laneCount; i++)
        {
            pos += spacing;
            lanes[i] = OffsetSpline(pos, meshDetail);
            pos += spacing;
        }
    }
    ...
    public void GenerateCollision(ExtrudeShape shape, float detail)
    {
        List<OrientationPoint> path = new List<OrientationPoint>();
        int refPointCount = Mathf.FloorToInt(ControlPointCount * detail);
        ...
        GetComponent<MeshCollider>().sharedMesh = TrackGenerator.Extrude(shape, path.ToArray());
    }
